...
CREATE TABLE IF NOT EXISTS claim_workflow (
    ...
    claim_id        UUID NOT NULL REFERENCES evidence_core.claims(id) ON DELETE CASCADE,
...
ALTER TABLE evidence_core.claims ADD COLUMN IF NOT EXISTS workflow_state workflow_state NOT NULL DEFAULT 'draft';
...
CREATE TABLE IF NOT EXISTS review_tasks (
    ...
    claim_id        UUID REFERENCES evidence_core.claims(id),
    evidence_node_id UUID REFERENCES evidence_core.evidence_nodes(id),
...
CREATE POLICY claim_workflow_org_access ON claim_workflow
    USING (claim_id IN (
        SELECT id FROM evidence_core.claims WHERE organization_id = (auth.jwt() ->> 'organization_id'::text)::UUID
    ));
