...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/v1/workspace/payments`, { headers: token ? { Authorization: `Bearer ${token}` } : {} })
      .then(r => r.json()).then(j => setData(j.data)).finally(() => setLoading(false))
  }, [user])
  if (!user) return <div style={{padding:40,textAlign:'center',color:'var(--txd)'}}>Sign in to view Payments</div>
  return (
    <div>
      <h1 style={{fontSize:20,fontWeight:800,margin:0}}>Payments</h1>
      <p style={{fontSize:13,color:'var(--txdd)',margin:'4px 0 24px'}}>Settlement and fee distribution</p>
