...
export const POST = withAsyncTracing(
  withAuth(async (request, user, params) => {
    try {
      ...
      if (!targetProfile) throw new ApiError(404, 'User not found. They must create an account first.')
      ...
      if (existing) throw new ApiError(409, `User is already a member (status: ${existing.status})`)
      ...
      if (insertErr) throw new ApiError(500, 'Failed to invite user', insertErr.message)

      // ── Cross-engine hooks (fire-and-forget) ──────────────────────────
      console.log(JSON.stringify({
        type: 'domain_event',
        event: {
          type: 'OrganizationMembershipAdded',
          payload: { membershipId: membership.id, organizationId: orgId, userId: targetProfile.id, role: parsed.data.role },
  ...
  SPAN_API_REQUEST,
  { attributes: { 'kadarn.api.route': 'organizations.invite', 'kadarn.api.method': 'POST' } },
)
