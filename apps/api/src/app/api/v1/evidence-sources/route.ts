// ==========================================================================
// KADARN v2 — Sprint 1: Evidence Sources List API
// ==========================================================================
// GET  /api/v1/evidence-sources — List sources (with filters)
// POST /api/v1/evidence-sources — Create a source
// ==========================================================================
...
export const GET = withAuth(async (request, _user) => {
  try {
    ...
    if (error) throw new ApiError(500, 'Failed to fetch evidence sources');
...
export const POST = withAuth(async (request, _user) => {
  try {
    ...
    const { data, error } = await supabase.from('evidence_sources').insert(parsed.data).select().single();
    if (error) {
